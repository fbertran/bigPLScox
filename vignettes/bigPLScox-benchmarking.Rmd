...
gpls_fit <- coxgpls(train_x, train_time, train_status, ncomp = 5, ind.block.x = c(3, 10))
...
lp_gpls <- predict(gpls_fit, newdata = test_x, type = "lp")
lp_big <- predict(big_fit, newdata = test_x, type = "link")
lp_gd <- predict(gd_fit, newdata = test_x, type = "link")
lp_pls <- as.numeric(test_x %*% pls_fit$CoeffC[, 1:5])

surv_test <- survival::Surv(test_time, test_status)
acc_results <- data.frame(
  method = c("coxgpls", "big_pls_cox", "big_pls_cox_gd", "plsRcox"),
  c_index = c(
    survival::survConcordance(surv_test ~ lp_gpls)$concordance,
    survival::survConcordance(surv_test ~ lp_big)$concordance,
    survival::survConcordance(surv_test ~ lp_gd)$concordance,
    survival::survConcordance(surv_test ~ lp_pls)$concordance
  )
)
acc_results
```
