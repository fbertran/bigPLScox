...
if (requireNamespace("bigmemory", quietly = TRUE)) {
  ...
  big_fit <- big_pls_cox(X_train, Y_train, status_train, ncomp = 4)
  big_fit$cox_fit
}
```
...
```{r, cache=TRUE, eval=LOCAL}
if (requireNamespace("bigmemory", quietly = TRUE)) {
  gd_fit <- big_pls_cox_gd(X_big_train, Y_train, status_train, ncomp = 4, max_iter = 2000)
  gd_fit$cox_fit
}
```
...
if (requireNamespace("bigmemory", quietly = TRUE)) {
  risk_table <- data.frame(
    ...
    big_pls = predict(big_fit, newdata = X_test, type = "link", comps = 1:4),
