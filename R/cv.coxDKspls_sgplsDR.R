...
#' variables passed to \code{\link{coxDKspls_sgplsDR}}'s \code{Xplan} argument,
#' \item\code{time} passed to \code{\link{coxDKspls_sgplsDR}}'s \code{time} argument,
#' \item\code{status} \code{\link{coxDKspls_sgplsDR}}'s \code{status} argument.  }
...
#' @param \dots Other arguments to pass to \code{\link{coxDKspls_sgplsDR}}.
...
#' @seealso See Also \code{\link{coxDKspls_sgplsDR}}
...
cv.coxDKspls_sgplsDR =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""),coxDKspls_sgplsDR(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, allres=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep="")),file=paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$CoeffCFull

      newdataAUC <- kernlab::kernelMatrix(kernel=get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$kernDKspls_sgplsDR_mod,
                                 x=scale(tsdata$x,
                                       get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                 y=scale(trdata$x,
                                       get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal))
      alldataAUC <- kernlab::kernelMatrix(kernel=get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$kernDKspls_sgplsDR_mod,
                                 x=scale(x,
                                       get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                 y=scale(trdata$x,
      ...
      for(jj in 1:nt){
        ...
        newxdata=(predict.gPLS(get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$DKspls_sgplsDR_mod,
                                  newdata=as.data.frame(newdataAUC),scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$tt_DKspls_sgplsDR))[,1:jj,drop=FALSE]
        
        allxdata=(predict.gPLS(get(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep=""))$DKspls_sgplsDR_mod,
                                  newdata=as.data.frame(alldataAUC),scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        if(jj==1){
      ...
      rm(list=c(paste("cv.coxDKspls_sgplsDR_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
      for(ind in 1:number_ind) {
