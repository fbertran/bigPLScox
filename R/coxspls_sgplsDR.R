...
#' 
#' @aliases coxspls_sgplsDR coxspls_sgplsDR.default coxspls_sgplsDR.formula
...
#' @return If \code{allres=FALSE} : \item{cox_spls_sgplsDR}{Final Cox-model.} If
#' \code{allres=TRUE} : \item{tt_spls_sgplsDR}{PLSR components.} \item{cox_spls_sgplsDR}{Final
#' Cox-model.} \item{spls_sgplsDR_mod}{The PLSR model.}
...
#' 
#' @export coxspls_sgplsDR
coxspls_sgplsDR <- function (Xplan, ind.block.x = NULL, ...) UseMethod("coxspls_sgplsDR")

#' @rdname coxspls_sgplsDR
#' @export
coxspls_sgplsDR.formula <-
...
#    ind.block.x <- sapply(ind.block.x, function(x) {sum(attr(bbb,"assign") <= x)})
    ...
    NextMethod("coxspls_sgplsDR")
  }


#' @rdname coxspls_sgplsDR
#' @export
coxspls_sgplsDR.default <- 
  function (Xplan, time, time2, event, type, origin, typeres = "deviance", 
            ...
            keepX, plot = FALSE, allres = FALSE, ...) 
  {
    ...
    if (mf2$ncomp == 0) {
      ...
      spls_sgplsDR_mod <- eval(mf2)
    }
    tt_spls_sgplsDR <- data.frame(spls_sgplsDR_mod$variates$X)
    if (mf2$ncomp > 0) {
      colnames(tt_spls_sgplsDR) <- paste("dim", 1:ncol(tt_spls_sgplsDR), sep = ".")
    }
    if (mf2$ncomp == 0) {
      ...
      mf2b$data <- tt_spls_sgplsDR
      mf2b[[1L]] <- as.name("coxph")
      cox_spls_sgplsDR <- eval(mf2b, parent.frame())
      cox_spls_sgplsDR$call$data <- as.name("tt_spls_sgplsDR")
    }
    else {
      mf2b <- match.call(expand.dots = TRUE)
      m2b <- match(c(head(names(as.list(args(coxph))), -2), 
    ...
    if (!allres) {
      ...
      if (mf2$ncomp > 0) {
        for (iii in 1:ncomp) {
          ...
          mf2b <- mf2b[c(1L, m2b)]
          mf2b$formula <- as.formula(YCsurv ~ .)
          mf2b$data <- tt_spls_sgplsDR[, 1:iii, drop = FALSE]
          mf2b[[1L]] <- as.name("coxph")
          cox_spls_sgplsDR <- eval(mf2b, parent.frame())
          cox_spls_sgplsDR$call$data <- as.name("tt_spls_sgplsDR")
          CoeffCFull[, iii] <- c(cox_spls_sgplsDR$coefficients, 
                                 rep(NA, ncomp - iii))
        }
      }
      return(list(tt_spls_sgplsDR = tt_spls_sgplsDR, cox_spls_sgplsDR = cox_spls_sgplsDR, 
                  spls_sgplsDR_mod = spls_sgplsDR_mod, 
