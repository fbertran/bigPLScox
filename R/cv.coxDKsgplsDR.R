...
#' 
#' This function cross-validates \link{coxDKsgplsDR} models.\cr
#' 
...
#' variables passed to \code{\link{coxDKsgplsDR}}'s \code{Xplan} argument,
#' \item\code{time} passed to \code{\link{coxDKsgplsDR}}'s \code{time} argument,
#' \item\code{status} \code{\link{coxDKsgplsDR}}'s \code{status} argument.  }
...
#' @param \dots Other arguments to pass to \code{\link{coxDKsgplsDR}}.
...
#' @seealso See Also \code{\link{coxDKsgplsDR}}
...
cv.coxDKsgplsDR =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""),coxDKsgplsDR(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, allres=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep="")),file=paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$CoeffCFull

      newdataAUC <- kernlab::kernelMatrix(kernel=get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$kernDKsgplsDR_mod,
                                 x=scale(tsdata$x,
                                       get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                 y=scale(trdata$x,
                                       get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal))
      alldataAUC <- kernlab::kernelMatrix(kernel=get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$kernDKsgplsDR_mod,
                                 x=scale(x,
                                       get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                 y=scale(trdata$x,
      ...
      for(jj in 1:nt){
        ...
        newxdata=(predict.gPLS(get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$DKsgplsDR_mod,
                                  newdata=as.data.frame(newdataAUC),scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$tt_DKsgplsDR))[,1:jj,drop=FALSE]
        
        allxdata=(predict.gPLS(get(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep=""))$DKsgplsDR_mod,
      ...
      rm(list=c(paste("cv.coxDKsgplsDR_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
      for(ind in 1:number_ind) {
