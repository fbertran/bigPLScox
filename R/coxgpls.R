...
#' 
#' @aliases coxgpls coxgpls.default coxgpls.formula
...
#' @return If \code{allres=FALSE} : \item{cox_gpls}{Final Cox-model.} If
#' \code{allres=TRUE} : \item{tt_gpls}{PLSR components.} \item{cox_gpls}{Final
#' Cox-model.} \item{gpls_mod}{The PLSR model.}
...
#' 
#' @export coxgpls
coxgpls <- function (Xplan, ...) UseMethod("coxgpls")
...
coxgpls.default <- 
            ...
            keepX, plot = FALSE, allres = FALSE, ...)
  {
    ...
    tt_gpls <- data.frame(gpls_mod$variates$X)
    if (mf2$ncomp > 0) {
      colnames(tt_gpls) <- paste("dim", 1:ncol(tt_gpls), sep = ".")
    }
    if (mf2$ncomp == 0) {
    ...
    if (!allres) {
      ...
      if (mf2$ncomp > 0) {
        for (iii in 1:ncomp) {
          ...
          mf2b <- mf2b[c(1L, m2b)]
          mf2b$formula <- as.formula(YCsurv ~ .)
          mf2b$data <- tt_gpls[, 1:iii, drop = FALSE]
          mf2b[[1L]] <- as.name("coxph")
          cox_gpls <- eval(mf2b, parent.frame())
          cox_gpls$call$data <- as.name("tt_gpls")
