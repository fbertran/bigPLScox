...
#' 
#' This function cross-validates \link{coxDKgplsDR} models.\cr
#' 
...
#' variables passed to \code{\link{coxDKgplsDR}}'s \code{Xplan} argument,
#' \item\code{time} passed to \code{\link{coxDKgplsDR}}'s \code{time} argument,
#' \item\code{status} \code{\link{coxDKgplsDR}}'s \code{status} argument.  }
...
#' @param \dots Other arguments to pass to \code{\link{coxDKgplsDR}}.
...
#' @seealso See Also \code{\link{coxDKgplsDR}}
...
cv.coxDKgplsDR =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""),coxDKgplsDR(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, allres=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep="")),file=paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$CoeffCFull

      newdataAUC <- kernlab::kernelMatrix(kernel=get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$kernDKgplsDR_mod,
                                 x=scale(tsdata$x,
                                       get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                 y=scale(trdata$x,
                                       get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal))
      alldataAUC <- kernlab::kernelMatrix(kernel=get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$kernDKgplsDR_mod,
                                 x=scale(x,
                                       get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
                                       get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal),
                                 y=scale(trdata$x,
      ...
      for(jj in 1:nt){
        ...
        newxdata=(predict.gPLS(get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$DKgplsDR_mod,
                                  newdata=as.data.frame(newdataAUC),scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$tt_DKgplsDR))[,1:jj,drop=FALSE]
        
        allxdata=(predict.gPLS(get(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep=""))$DKgplsDR_mod,
      ...
      rm(list=c(paste("cv.coxDKgplsDR_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
      for(ind in 1:number_ind) {
