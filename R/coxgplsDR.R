...
#' 
#' @aliases coxgplsDR coxgplsDR.default coxgplsDR.formula
...
#' @return If \code{allres=FALSE} : \item{cox_gplsDR}{Final Cox-model.} If
#' \code{allres=TRUE} : \item{tt_gplsDR}{PLSR components.} \item{cox_gplsDR}{Final
#' Cox-model.} \item{gplsDR_mod}{The PLSR model.}
...
#' 
#' @export coxgplsDR
coxgplsDR <- function (Xplan, ...) UseMethod("coxgplsDR")
...
coxgplsDR.default <- 
            ...
            keepX, plot = FALSE, allres = FALSE, ...) 
  {
    ...
    tt_gplsDR <- data.frame(gplsDR_mod$variates$X)
    if (mf2$ncomp > 0) {
      colnames(tt_gplsDR) <- paste("dim", 1:ncol(tt_gplsDR), sep = ".")
    }
    if (mf2$ncomp == 0) {
      ...
      cox_gplsDR <- eval(mf2b, parent.frame())
      cox_gplsDR$call$data <- as.name("tt_gplsDR")
    }
    else {
      mf2b <- match.call(expand.dots = TRUE)
      m2b <- match(c(head(names(as.list(args(survival::coxph))), -2), 
    ...
    if (!allres) {
      ...
      if (mf2$ncomp > 0) {
        for (iii in 1:ncomp) {
          ...
          mf2b <- mf2b[c(1L, m2b)]
          mf2b$formula <- as.formula(YCsurv ~ .)
          mf2b$data <- tt_gplsDR[, 1:iii, drop = FALSE]
          mf2b[[1L]] <- as.name("coxph")
          cox_gplsDR <- eval(mf2b, parent.frame())
          cox_gplsDR$call$data <- as.name("tt_gplsDR")
          CoeffCFull[, iii] <- c(cox_gplsDR$coefficients,
                                 rep(NA, ncomp - iii))
        }
      }
      res <- list(tt_gplsDR = tt_gplsDR, cox_gplsDR = cox_gplsDR, gplsDR_mod = gplsDR_mod,
                  XplanScal = XplanScal, XplanCent = XplanCent,
                  CoeffCFull = CoeffCFull)
      res$XplanTrain <- as.matrix(Xplan)
      class(res) <- c("coxgplsDR", "cox_pls_legacy")
      return(res)
    }
  }
