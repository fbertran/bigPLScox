...
predict.sgPLS <- function (object, newdata, scale.X=TRUE, scale.Y=TRUE, ...)
{
  ...
  b <- object$loadings$Y
  if (is.null(a)) {
    stop("The sgPLS fit does not contain X loadings; cannot proceed with prediction.")
  }
  if (is.null(b)) {
    stop("The sgPLS fit does not contain Y loadings; cannot proceed with prediction.")
  }
  if (is.null(Y)) {
    stop("The sgPLS fit does not contain the response used during training; cannot compute predictions.")
  }
