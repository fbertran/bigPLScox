...
#' 
#' This function cross-validates \link{coxsgpls} models.\cr
#' 
...
#' variables passed to \code{\link{coxsgpls}}'s \code{Xplan} argument,
#' \item\code{time} passed to \code{\link{coxsgpls}}'s \code{time} argument,
#' \item\code{status} \code{\link{coxsgpls}}'s \code{status} argument.  }
...
#' @param \dots Other arguments to pass to \code{\link{coxsgpls}}.
...
#' @seealso See Also \code{\link{coxsgpls}}
...
cv.coxsgpls =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxsgpls_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""),coxsgpls(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, allres=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep="")),file=paste("cv.coxsgpls_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.coxsgpls_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$CoeffCFull
      
      tsdata_x_scale <- scale(tsdata$x,
            center=get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$XplanCent,
            scale=get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$XplanScal)
      x_scale <- scale(x,
            get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$XplanCent,
            get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$XplanScal)
      
      for(jj in 1:nt){
        
        newxdata=(predict.gPLS(get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$sgpls_mod,
                                  newdata=tsdata_x_scale,scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$tt_sgpls))[,1:jj,drop=FALSE]
        
        allxdata=(predict.gPLS(get(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep=""))$sgpls_mod,
      ...
      rm(list=c(paste("cv.coxsgpls_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
      for(ind in 1:number_ind) {
