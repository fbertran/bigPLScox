...
#' variables passed to \code{\link{coxsgplsDR}}'s \code{Xplan} argument,
#' \item\code{time} passed to \code{\link{coxsgplsDR}}'s \code{time} argument,
#' \item\code{status} \code{\link{coxsgplsDR}}'s \code{status} argument.  }
...
#' @param \dots Other arguments to pass to \code{\link{coxsgplsDR}}.
...
#' @seealso See Also \code{\link{coxsgplsDR}}
...
cv.coxsgplsDR =
            ...
            details=FALSE, namedataset="data", save=FALSE, verbose=TRUE,...)
  {
    ...
    for (i in seq(nfold)) {
      ...
      if(!file.exists(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,".RData",sep=""))){
        assign(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""),coxsgplsDR(Xplan=trdata$x, time=trdata$time, event=trdata$status, ncomp=nt, allres=TRUE, verbose=verbose, ...))
        if(save){save(list=c(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep="")),file=paste("cv.coxsgplsDR_",namedataset,"_folds_",i,".RData",sep=""))}
      } else {
        load(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,".RData",sep=""))
      }
      
      coeffit <- get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$CoeffCFull
      
      tsdata_x_scale <- scale(tsdata$x,
            center=get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
            scale=get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal)
      x_scale <- scale(x,
            get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanCent,
            get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$XplanScal)
      
      for(jj in 1:nt){
        
        newxdata=(predict.gPLS(get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$sgplsDR_mod,
                                  newdata=tsdata_x_scale,scale.X=FALSE,scale.Y=FALSE)$variates)[,1:jj,drop=FALSE]
        oldxdata=(as.matrix(get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$tt_sgplsDR))[,1:jj,drop=FALSE]
        
        allxdata=(predict.gPLS(get(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep=""))$sgplsDR_mod,
      ...
      rm(list=c(paste("cv.coxsgplsDR_",namedataset,"_folds_",i,sep="")))
    }
    if(allCVcrit){
      for(ind in 1:number_ind) {
