...
#' 
#' @aliases coxDKspls_sgplsDR coxDKspls_sgplsDR.default coxDKspls_sgplsDR.formula
...
#    ind.block.x <- sapply(ind.block.x, function(x) {sum(attr(bbb,"assign") <= x)})
    ...
    NextMethod("coxDKspls_sgplsDR")
  }


#' @rdname coxDKspls_sgplsDR
#' @export
coxDKspls_sgplsDR.default <- 
  function (Xplan, time, time2, event, type, origin, typeres = "deviance",
            ...
            kernel="rbfdot", hyperkernel, verbose=FALSE, ...)
  {
    ...
    if(missing(hyperkernel)){if(kernel=="rbfdot"){
      ...
      srangeDKspls_sgplsDR_mod <- eval(mf1rc, parent.frame())
      hyperkernel=list(sigma = srangeDKspls_sgplsDR_mod[2])
      if(verbose){cat("Estimated_sigma ",srangeDKspls_sgplsDR_mod[2],"\n")}
    ...
    kernDKspls_sgplsDR_mod <- eval(call(as.character(quote(kernel1rc))))
    ...
    if (mf2$ncomp == 0) {
      ...
      DKspls_sgplsDR_mod <- eval(mf2)
    }
    tt_DKspls_sgplsDR <- data.frame(DKspls_sgplsDR_mod$variates$X)
    if (mf2$ncomp > 0) {
      colnames(tt_DKspls_sgplsDR) <- paste("dim", 1:ncol(tt_DKspls_sgplsDR), sep = ".")
    }
    if (mf2$ncomp == 0) {
      ...
      mf2b$data <- tt_DKspls_sgplsDR
      mf2b[[1L]] <- as.name("coxph")
      cox_DKspls_sgplsDR <- eval(mf2b, parent.frame())
      cox_DKspls_sgplsDR$call$data <- as.name("tt_DKspls_sgplsDR")
    }
    else {
      mf2b <- match.call(expand.dots = TRUE)
      m2b <- match(c(head(names(as.list(args(survival::coxph))), -2), 
    ...
    if (!allres) {
      ...
      if (mf2$ncomp > 0) {
        for (iii in 1:ncomp) {
          ...
          mf2b <- mf2b[c(1L, m2b)]
          mf2b$formula <- as.formula(YCsurv ~ .)
          mf2b$data <- tt_DKspls_sgplsDR[, 1:iii, drop = FALSE]
          mf2b[[1L]] <- as.name("coxph")
          cox_DKspls_sgplsDR <- eval(mf2b, parent.frame())
          cox_DKspls_sgplsDR$call$data <- as.name("tt_DKspls_sgplsDR")
          CoeffCFull[, iii] <- c(cox_DKspls_sgplsDR$coefficients,
                                 rep(NA, ncomp - iii))
        }
      }
      res <- list(tt_DKspls_sgplsDR = tt_DKspls_sgplsDR, cox_DKspls_sgplsDR = cox_DKspls_sgplsDR,
                  DKspls_sgplsDR_mod = DKspls_sgplsDR_mod,
                  XplanScal = XplanScal, XplanCent = XplanCent, CoeffCFull = CoeffCFull,
                  kernDKspls_sgplsDR_mod = kernDKspls_sgplsDR_mod)
      res$XplanTrain <- as.matrix(Xplan)
      class(res) <- c("coxDKspls_sgplsDR", "cox_pls_legacy")
      return(res)
    }
  }
